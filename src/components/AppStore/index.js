...
class AppStore extends Component {
  state = {
    category: appsList[0].category,
    list: appsList,
  }

  updateCategory = id => {
    const {category} = this.state
    const newList = appsList.filter(each => each.category === category)

    this.setState({category: id, list: newList})
  }

  onPress = event => {
    const {list} = this.state
    const p = list.filter(each =>
  ...
  render() {
    ...
    return (
      <div className="bg">
        ...
        <ul className="bg5">
          {list.map(each => (
            <AppItem key={each.appId} appsList={each} />
          ))}
        </ul>
      </div>
    )
